...
| | Python | C++ | Java |
|---|---|---|---|
| 방식 | **이름 규약**(던더)만 지키면 문법이 자동 호출 | `operator` **키워드**로 연산자 오버로딩 | **연산자** 오버로딩 없음 — 정해진 이름을 **직접 호출** |
...
**"오버로딩"이라는 말의 두 갈래** — 오해하기 쉬운 지점이라 따로 짚습니다:

| | 메서드 오버로딩 (같은 이름, 다른 매개변수) | 연산자 오버로딩 (`==`·`+` 재정의) |
|---|---|---|
| C++ | ✅ | ✅ (`operator`) |
| Java | ✅ — `println(int)`/`println(String)` 처럼 일상적으로 쓴다 | ❌ (문자열 `+` 만 예외) |
| Python | ❌ — 같은 이름으로 다시 `def` 하면 **마지막 것이 덮어쓴다** | ✅ (던더) |

Java 에 "없다"는 것은 **연산자** 오버로딩입니다 — 메서드 오버로딩은 오히려 Java 의 일상입니다.
반대로 Python 은 연산자 쪽은 던더로 열려 있지만 **메서드 오버로딩이 없어서**, 기본 인자와
유니온 타입(`Hand | str`)으로 대신합니다 — `game.py` 의 `play()` 가 정확히 그 사례입니다
(Java/C++ 라면 `play(String)` / `play(Hand)` 두 개로 나눴을 것).
...
에러 라벨('oom' 같은)은 **마지막으로 잡은 계층**이 붙입니다([05-디버깅](05-디버깅.md)에서
자세히 다룹니다). 그런데 `try/except` 를 쓰는 순간 — **우리가 바로 그 계층이 됩니다.** 우리가 붙인(또는 삼킨)
...
- 라벨을 바꿀 때는 `raise ... from exc` — [05-디버깅](05-디버깅.md)의 "사인만 남고 범인이
  지워지는" 문제를 **우리 코드에서 만들지 않는** 방법이다.
